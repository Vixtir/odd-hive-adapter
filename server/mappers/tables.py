...
from server.mappers.metadata import MetadataExtractor
from server.mappers.columns.main import map_column
from server.mappers.oddrn import get_table_oddrn, get_owner_oddrn
...
def map_hive_table(raw_table_data: List[Tuple[str]],
    ...
    owner_oddrn = get_owner_oddrn(owner)

    columns = flatten([map_column(column_data, table_oddrn, __get_one_stat(column_stats, column_data))
    ...
    try:
        ...
        logging.warning("Problems with DataEntity JSON serialization. Returning: {}.")
        return {}


def __get_value(raw_table_data, row_value) -> str:
    try:
        value = next(i[i.index(j)+1] for i in raw_table_data for j in i if j == row_value)
        return value.strip()
    except StopIteration as e:
        logging.warning("There was an error during getting value for row. "
...
def __get_one_stat(column_stats, column_data) -> Dict[str, Any]:
    try:
        ...
        logging.warning("There was an exception raised during getting data for the column. ")
        return None
