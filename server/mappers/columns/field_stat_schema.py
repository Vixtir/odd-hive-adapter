...
def _mapper_decimal(raw_column_stat: Dict[str, Any]):
    ...
    return {
        'low_value': DEFAULT_VALUE if (raw_column_stat['min'] == "") \
            else _digit_checker(raw_column_stat['min'], float) / float(decimal_scale.replace(',', '.')),
        'high_value': DEFAULT_VALUE if (raw_column_stat['max'] == "") \
