...
def map_column_stats(unmapped_column_stats: List[Dict[str, Any]]) -> Iterable[Tuple[str, Dict[str, Any]]]:
    ...
    """
    return [__map_column_stat(raw_column_stat) for raw_column_stat in unmapped_column_stats]
...
def __map_column_stat(raw_column_stat: Dict[str, Any]) -> (str, Dict[str, Any]):
    ...
    try:
        ...
        return raw_column_stat['# col_name'], {statistics_data['field_name']: mapper_fn(raw_column_stat)}
    except KeyError:
        return None, {}
...
def map_column(raw_column_data: Dict[str, Any],
    ...
    try:
        ...
        return __map_column(table_oddrn=table_oddrn,
        ...
        logging.warning("There was an error during type parsing. "
                        f"Column data: \"{raw_column_data}\". Table oddrn: \"{table_oddrn}\". "
                        f"Reason: \"{e}\"")
        return []


def __map_column(table_oddrn: str,
    ...
    if hive_type in ['array', 'struct', 'union']:
        for children in type_parsed['children']:
            result.extend(__map_column(table_oddrn=table_oddrn,
